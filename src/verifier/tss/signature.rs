...
impl SignatureVerifier {
    ...
    pub fn verify(
        ...
        let pub_key = PublicKey::from_sec1_bytes(&self.group_pub_key)
        ...
        let challenge_scalar = self.compute_challenge(r_addr_bytes, hashed_message)?;
    ...
    fn compute_challenge(&self, r_addr: [u8; 20], msg_hash: [u8; 32]) -> anyhow::Result<Scalar> {
        let mut hasher = Keccak::v256();
    ...
    fn point_to_address(&self, point: &AffinePoint) -> anyhow::Result<[u8; 20]> {
...
mod tests {
    ...
    fn test_band_tss_verification_1() {
        ...
        let verifier = SignatureVerifier::new(group_pk);
        ...
        // 3. Execution
        let result = verifier.verify(&raw_data, &r_address, &signature_s);
