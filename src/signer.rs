...
pub fn public_key_to_soroban_address(public_key: &[u8]) -> anyhow::Result<String> {
    if public_key.len() != 32 {
        return Err(anyhow!(
            "Invalid public key length for Stellar address. Expected 32 bytes (Ed25519), got {}",
            public_key.len()
        ));
    }

    // Stellar StrKey encoding for ED25519 public key (G... address)
    // Version byte: 6 << 3 = 48
    ...
    Ok(stellar_base32_encode(&payload))
}
